...
import { evaluateReport, thresholds } from "./resource-thresholds.ts";
...
if (prove) {
  const rejected = result.failures.some(
    (item) => item.metric === "trial1.settledIdle.peakRssBytes",
  );
  console.log(
    rejected
      ? "PASS: a deliberate one-byte regression was rejected."
      : "FAIL: regression was missed.",
  );
