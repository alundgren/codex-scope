...
export async function completed(page, expected = null) {
  ...
  assert(!/timed out|could not|Searching…/.test(notice), `Measured query must succeed: ${notice}`);
  await page.evaluate(() => new Promise(resolve => requestAnimationFrame(resolve)));
}

export function distribution(values) {
  const sorted = values.toSorted((a, b) => a - b);
  return { operations: values.length, minimum: sorted[0], maximum: sorted.at(-1),
    p95: sorted[Math.ceil(sorted.length * .95) - 1], mean: values.reduce((sum, value) => sum + value, 0) / values.length };
}

export async function interactions(page, state) {
  const queries = [], keys = [];
  for (const text of ['population', 'odd', 'literal [a.*]%_', 'missing literal', '2026-09-11t18', 'POPULATION', 'even', 'session-39', '.*', '']) {
    ...
    try { await completed(page); } catch (error) { error.message = `Synthetic query ${JSON.stringify(text)}: ${error.message}`; throw error; }
  ...
  for (const key of ['End', 'ArrowUp', 'PageUp', 'PageDown', 'Home', 'ArrowDown', 'ArrowRight', 'ArrowLeft', 'End', 'ArrowUp']) {
    ...
    await completed(page, { id: history.first.id + Math.min(history.total - 1, position), position });
    keys.push(performance.now() - started);
  }
  await rapidScrub(page);
  return { searchMs: distribution(queries), keyboardMs: distribution(keys), pointerMoves: 120,
    summaryRows: await page.locator('.event').count(), tickNodes: await page.locator('.tick').count() };
}

export async function rapidScrub(page) {
