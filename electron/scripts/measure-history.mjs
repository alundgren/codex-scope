...
const report = { date: new Date().toISOString(), environment: { platform: os.platform(), release: os.release(), cpus: os.cpus().map(cpu => cpu.model), memory: os.totalmem() },
  method: 'Actual Electron under Xvfb, no screenshots or recording. Sample all process-group members and descendants at 250 ms. Summed RSS includes shared pages; final PSS is a single aggregate snapshot. CPU 100% is one core. Workloads include test-driver serialization in main. Disk samples include the live rollback journal before commit.', workloads: {} };
try {
  ...
  await app.evaluate(() => {
    ...
    globalThis.delayTimer = setInterval(() => { const now = performance.now(); globalThis.mainDelay = Math.max(globalThis.mainDelay, now - last - 20); last = now; }, 20);
  });
  const status = () => app.evaluate(() => globalThis.scopeHistory.snapshot());
  async function feed(count, template, batch, delay) {
    await app.evaluate(async (_electron, { count, message, batch, delay }) => {
      const history = globalThis.scopeHistory;
      for (let index = 0; index < count;) {
        for (let sent = 0; sent < batch && index < count; sent++, index++) history.append(history.generation, history.status.connectionId,
          JSON.stringify({ ...message, connection_id: history.status.connectionId, sequence: globalThis.measureSequence++ }));
        await new Promise(resolve => setTimeout(resolve, delay));
      }
      const deadline = performance.now() + 5000;
      while (history.sending && performance.now() < deadline) await new Promise(resolve => setTimeout(resolve, 10));
    }, { count, message: source[template], batch, delay });
  }
  async function workload(name, count, template, batch, delay) {
    report.workloads[name] = await sample(app, 0, () => feed(count, template, batch, delay));
    report.workloads[name].input = { count, payloadBytes: source[template].payload_bytes, batch, delayMs: delay };
    report.workloads[name].history = await status();
    report.workloads[name].maximumMainTimerDelayMs = await app.evaluate(() => { const value = globalThis.mainDelay; globalThis.mainDelay = 0; return value; });
  ...
  for (let index = 0; index < 30; index++) {
    ...
    await page.evaluate(async ({ generation, id }) => window.scope.inspect(generation, id, 5), { generation: value.generation, id: value.first.id + index });
    times.push(performance.now() - start);
  }
  report.inspectionMs = { maximum: Math.max(...times), p95: times.toSorted((a, b) => a - b)[28], operations: times.length };
