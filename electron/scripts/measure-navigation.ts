...
import { interactions } from "./interaction-metrics.ts";
...
async function feed(count: number, maximum = false) {
  await app.evaluate(
    async (_electron, { count, message, maximum }) => {
      ...
      for (let index = 0; index < count; index++) {
        ...
        if (!maximum) {
          ...
          frame = {
            ...
            received_at: "2026-09-11T18:00:00.000Z",
          };
        }
        history.append(
          history.generation,
        ...
        if (maximum || index % 8 === 7)
          await new Promise((resolve) => setTimeout(resolve, maximum ? 65 : 45));
      }
...
try {
  ...
  for (const [label, added] of [
    ...
    ["3000", 2000],
    ["10000", 7000],
    ["retentionRepeat", 10000],
  ] as const) {
    await record(`capture${label}`, () => feed(added));
    let timings;
    await record(`navigate${label}`, async () => {
      timings = await interactions(page, status);
    });
    report.workloads[`navigate${label}`].timings = timings;
  }
  await record("maximumPayloadCapture", () => feed(240, true));
  let timings;
  await record("maximumPayloadNavigation", async () => {
    timings = await interactions(page, status);
  });
