...
try {
  ...
  for (const [name, target] of Object.entries(targets)) {
    ...
    for (let trial = 0; trial < 3; trial++) {
      ...
      const app = await _electron.launch({ args: [target, '--fixtures-only'], chromiumSandbox: true });
      try {
        ...
        for (let step = 0; step < 16 && !await page.locator('button[data-event="4"]').count(); step++) await page.locator('button[data-event]').first().click();
