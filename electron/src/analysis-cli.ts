...
function cliArgs(model: string, effort: string, directory: string): string[] {
...
export async function runAnalysisCli(options: AnalysisCliOptions): Promise<AnalysisCliResult> {
  ...
  if (!validEffort(options.effort)) throw new Error("Choose a supported reasoning effort.");
  if (
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      [
        ...
        ...cliArgs(options.model, options.effort, directory),
      ],
      {
        cwd: directory,
        detached: true,
