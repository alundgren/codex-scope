...
function parseRecording(bytes: Uint8Array) {
  ...
  for (const line of source.split("\n")) {
    ...
    if (
      ...
      payloadBytes + Buffer.byteLength(raw) > MAX_RECORDING_BYTES
    ) {
      drops.capacity++;
      continue;
    }
...
async function loadRecording(path: string) {
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > MAX_SOURCE_BYTES)
      throw new Error("Fixture recording is too large.");
...
function inspect(recording: Recording, id: number | null, rows: number) {
  if (
  ...
  const selected =
    id === null
      ? recording.events[Math.min(2, recording.events.length - 1)]
  ...
  return {
    ...
    rows: recording.events
      .slice(start, start + rows)
      .map(({ id, receivedAt, hook, session, preview }) => ({
        id,
        receivedAt,
        hook: label(hook),
...
export type Recording = ReturnType<typeof parseRecording>;
