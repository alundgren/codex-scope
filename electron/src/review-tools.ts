...
export class ReviewTools {
  ...
  async call(name: string, value: unknown, signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      if (a.action === "image") {
        const bytes = await this.review.toolImage(this.reviewId, a.id);
      ...
      if (a.action === "list" && entry.type === "tree") {
        const entries = await this.review.toolList(this.reviewId, entry.path, signal, entry.side);
        const result = [];
        for (const child of entries.slice(offset, offset + 10)) {
          ...
          result.push({ id, ...child, side: entry.side });
        }
        return text({
          entries: result,
      ...
      const content = await this.review.toolSource(this.reviewId, entry.path, signal, entry.side);
