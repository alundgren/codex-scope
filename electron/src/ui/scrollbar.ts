export function attachScrollbar(payload: HTMLElement, track: HTMLElement, thumb: HTMLElement) {
  let drag: { offset: number } | null = null;
  ...
  function update() {
    ...
    const thumbHeight = Math.min(
      height,
      Math.max(32, (height * payload.clientHeight) / Math.max(1, payload.scrollHeight)),
    );
  ...
  function move(event: PointerEvent) {
    ...
    payload.scrollTop =
      ((event.clientY - track.getBoundingClientRect().top - drag.offset) / Math.max(1, travel)) *
      maximum();
  }
  function release() {
    drag = null;
  ...
  track.addEventListener("keydown", (event) => {
    const changes: Record<string, number> = {
      ...
      ArrowLeft: -40,
      ArrowRight: 40,
      PageUp: -payload.clientHeight * 0.8,
      PageDown: payload.clientHeight * 0.8,
      Home: -payload.scrollHeight,
      End: payload.scrollHeight,
    };
    if (Object.hasOwn(changes, event.key)) {
      event.preventDefault();
      payload.scrollTop += changes[event.key];
    }
  });
  track.addEventListener(
    ...
    (event) => {
      ...
      payload.scrollTop +=
        event.deltaY *
        (event.deltaMode === 1 ? 26 : event.deltaMode === 2 ? payload.clientHeight : 1);
    },
    { passive: false },
  );
