...
const messages = (await readFile('fixtures/journal.jsonl', 'utf8')).trim().split('\n').map(JSON.parse);
const original = id => messages[id].payload;
const offset = page => page.locator('#payload').evaluate(node => node.scrollTop);
const maximum = page => page.locator('#payload').evaluate(node => node.scrollHeight - node.clientHeight);

async function launch(testInfo, target = appPath) {
...
async function touchDrag(page, x, y, destination) {
  const client = await page.context().newCDPSession(page);
  await client.send('Emulation.setTouchEmulationEnabled', { enabled: true });
  await client.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x, y }] });
  for (let step = 1; step <= 8; step++) {
    await client.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x, y: y + (destination - y) * step / 8 }] });
...
test('recorded inspector walkthrough: selection, exact copy, failure recovery and every scrolling input', async ({}, info) => {
  ...
  try {
    ...
    await app.evaluate(({ clipboard }) => {
      ...
      clipboard.writeText = () => { clipboard.writeText = write; return Promise.reject(new Error('Synthetic clipboard failure')); };
    });
    ...
    await page.mouse.click(bounds.x + bounds.width / 2, bounds.y + bounds.height * .45);
    await expect.poll(() => offset(page)).toBeGreaterThan((await maximum(page)) * .35);
    const thumb = await page.locator('#thumb').boundingBox();
    const beforeGrab = await offset(page);
    await page.mouse.move(thumb.x + 5, thumb.y + thumb.height * .8);
    await page.mouse.down();
    expect(Math.abs(await offset(page) - beforeGrab)).toBeLessThan(2);
    await page.mouse.move(thumb.x + 5, thumb.y + thumb.height * .8 + 50, { steps: 10 });
    await page.mouse.up();
    ...
    expect(await page.locator('button[data-event="2"]').evaluate(node => node === document.activeElement)).toBe(true);
    await resize(app, 360, 640);
    await capture(page, info, 'minimum-window');
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    expect(await page.locator('#payload').evaluate(node => node.clientHeight)).toBeGreaterThan(30);
...
test('security boundaries deny Node, remote content, navigation, extra windows and invalid requests', async ({}, info) => {
  ...
  try {
    ...
    expect(await page.evaluate(() => [typeof require, typeof process, Object.keys(window.scope).sort()])).toEqual(['undefined', 'undefined', ['copyPayload', 'inspect']]);
    expect(await page.evaluate(async () => { try { await window.scope.inspect({}, 999); return false; } catch { return true; } })).toBe(true);
    expect(await page.evaluate(() => window.scope.copyPayload(-1))).toBe(false);
    const requests = await page.evaluate(() => Promise.allSettled(Array.from({ length: 100 }, () => window.scope.inspect(3, 5))).then(results => results.filter(item => item.status === 'fulfilled').length));
    expect(requests).toBe(1);
    const foreignRequest = await app.evaluate(async ({ BrowserWindow, session }, preload) => {
      const other = new BrowserWindow({ show: false, webPreferences: { sandbox: true, contextIsolation: true,
        nodeIntegration: false, session: session.fromPartition('synthetic'), preload } });
      try {
        await other.loadURL('scope://app/index.html');
        return await other.webContents.executeJavaScript('new Promise(resolve => setTimeout(resolve, 100)).then(() => window.scope.inspect(3, 5)).then(() => false, () => true)');
      } finally { other.destroy(); }
    }, path.join(appPath, 'src/preload.cjs'));
  ...
  } finally { await app.close(); await video.saveAs(info.outputPath('security.webm')); }
});
...
test('a stalled native clipboard write times out with one pending operation and recovers', async ({}, info) => {
  ...
  try {
    ...
    await app.evaluate(({ clipboard }) => {
      ...
      clipboard.writeText = () => {
        ...
        return new Promise(resolve => { globalThis.releaseClipboard = () => { clipboard.writeText = original; resolve(); }; });
      };
    });
    await page.locator('#copy').click();
    await expect(page.locator('#copy-status')).toContainText('Copy failed');
    ...
    await app.evaluate(() => { globalThis.releaseClipboard(); delete globalThis.releaseClipboard; delete globalThis.clipboardCalls; });
    await page.locator('#copy').click();
    await expect(page.locator('#copy')).toHaveText('Copied');
  ...
  } finally { await app.close(); await video.saveAs(info.outputPath('clipboard-timeout.webm')); }
});
...
test('long session and tool labels leave the complete payload and byte count usable', async ({}, info) => {
  ...
  const raw = JSON.stringify({ hook_event_name: 'PreToolUse', session_id: session, tool_name: tool, future_field: 'complete' });
  const message = { ...messages[3], sequence: 1, session_id: session, tool_name: tool, payload: raw, payload_bytes: Buffer.byteLength(raw) };
  await writeFile(path.join(temporary, 'fixtures/journal.jsonl'), [messages[0], message].map(JSON.stringify).join('\n'));
  ...
  try {
    ...
    expect(await page.locator('#payload').evaluate(node => node.clientHeight)).toBeGreaterThan(50);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    await page.locator('#copy').click();
    await expect(page.locator('#copy')).toHaveText('Copied');
    expect(await app.evaluate(({ clipboard }) => clipboard.readText())).toBe(raw);
    await capture(page, info, 'long-metadata');
  } finally { await app.close(); await video.saveAs(info.outputPath('long-metadata.webm')); await rm(temporary, { recursive: true, force: true }); }
});
...
test('a rejected oversized fixture remains absent while valid history stays usable', async ({}, info) => {
  ...
  await writeFile(path.join(temporary, 'fixtures/journal.jsonl'), [...messages, tooBig].map(JSON.stringify).join('\n') + '\n');
  const { app, page, video } = await launch(info, temporary);
  try {
    await page.waitForSelector('html[data-ready="true"]');
    await expect(page.locator('#notice')).toContainText('1 oversized');
    await expect(page.locator('#count')).toHaveText('5 fixtures');
    await select(page, 4);
    expect(await page.locator('#json').textContent()).toBe(original(4));
    await capture(page, info, 'oversized-rejected');
  } finally { await app.close(); await video.saveAs(info.outputPath('oversized.webm')); await rm(temporary, { recursive: true, force: true }); }
});
...
test('empty and unreadable fixtures explain their state, with recovery after restart', async ({}, info) => {
  ...
  await cp(appPath, temporary, { recursive: true });
  try {
    for (const [name, text] of [['empty', JSON.stringify(messages[0]) + '\n'], ['unreadable', 'invalid'], ['recovered', messages.map(JSON.stringify).join('\n') + '\n']]) {
      await writeFile(path.join(temporary, 'fixtures/journal.jsonl'), text);
      const { app, page, video } = await launch(info, temporary);
      try {
        if (name === 'unreadable') await expect(page.locator('#notice')).toContainText('could not be opened');
        else await page.waitForSelector('html[data-ready="true"]');
        if (name === 'empty') await expect(page.locator('#entries')).toContainText('No synthetic events');
      ...
      } finally { await app.close(); await video.saveAs(info.outputPath(`${name}.webm`)); }
    }
