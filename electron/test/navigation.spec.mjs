...
import { source, state, fault, launch, capture, frame, append, selected, expectSelected, search, touchDrag } from './navigation-helpers.mjs';
...
const events = Array.from({ length: 18 }, (_, index) => frame({ index, session: index % 2 ? sessionB : sessionA,
  hook: hooks[index % 3], message: `Synthetic navigation ${index} ${'plain '.repeat(40)}`, tail: index === 0 ? 'CAFÉ [a.*]%_ outside the preview' : `payload tail ${index}` }));
...
test('recorded filters: literal full text, full session IDs, several hooks, held offsets, identical counters and reset', async ({}, info) => {
  ...
  } finally { await app.close(); await video.saveAs(info.outputPath('filters-walkthrough.webm')); }
});
...
test('recorded scrubber: every input, newest history, frozen arrivals, eviction and pressure recovery', async ({}, info) => {
  ...
  try {
    ...
    await slider.press('Home'); await expectSelected(page, 1);
    for (const [key, id] of [['ArrowDown', 2], ['ArrowRight', 3], ['ArrowUp', 2], ['ArrowLeft', 1], ['PageDown', 6], ['PageUp', 1]]) {
      await slider.press(key); await expectSelected(page, id);
    }
    await slider.press('End'); await expect(page.locator('#mode')).toHaveText('Live');
    await slider.press('ArrowUp'); await expectSelected(page, 23);
    await expect(page.locator('#mode')).toContainText('History');
    await capture(page, info, 'newest-history');
    await slider.press('ArrowDown'); await expect(page.locator('#mode')).toHaveText('Live');
    await page.locator('#entries').hover(); await page.mouse.wheel(0, -120); await expectSelected(page, 23);
    await page.waitForTimeout(100); await page.mouse.wheel(0, -120); await expectSelected(page, 22);
    await page.waitForTimeout(100); await page.mouse.wheel(0, 120); await expectSelected(page, 23);
    let bounds = await slider.boundingBox();
    await page.mouse.move(bounds.x + 22, bounds.y + bounds.height * .2); await page.mouse.down();
    await page.mouse.move(bounds.x + 22, bounds.y + bounds.height * .6, { steps: 12 });
    ...
    await page.mouse.move(bounds.x + 22, bounds.y + bounds.height * .6);
    await expectSelected(page, 15);
    ...
    await touchDrag(page, bounds.x + 22, bounds.y + bounds.height * .8, bounds.y + bounds.height * .1);
    await expectSelected(page, 4);
    await touchDrag(page, bounds.x + 22, bounds.y + bounds.height * .1, bounds.y + bounds.height * .75);
    await expectSelected(page, 21);
    await capture(page, info, 'touch-scrub');
    await slider.press('End');
    await append(app, Array.from({ length: 130 }, () => source[4]), 75);
    bounds = await slider.boundingBox();
    await page.mouse.move(bounds.x + 22, bounds.y + bounds.height * .3); await page.mouse.down();
    await page.waitForTimeout(300);
    await append(app, Array.from({ length: 16 }, () => source[4]), 75);
    ...
    await slider.press('End'); await slider.press('ArrowUp');
    await capture(page, info, 'pressure-navigation');
    await fault(app, { disk: false }); await append(app, [source[4]], 80);
    await expect(page.locator('#notice')).not.toContainText('Storage pressure');
    await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].setContentSize(440, 820));
    await slider.press('Home'); await slider.press('PageDown'); await slider.press('End');
    bounds = await slider.boundingBox();
    await touchDrag(page, bounds.x + 22, bounds.y + bounds.height * .9, bounds.y + bounds.height * .4);
    await expect(page.locator('#mode')).toContainText('History');
    await capture(page, info, 'narrow-scrub-recovery');
    expect(await page.locator('.event').count()).toBeLessThanOrEqual(3);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally { await app.close(); await video.saveAs(info.outputPath('scrubber-walkthrough.webm')); }
});
...
test('recorded delayed queries, cancellation, timeout, Clear generations and bounded option paging recover', async ({}, info) => {
  ...
  try {
    ...
    await page.locator('#search').fill('retry'); await page.waitForTimeout(230);
    await page.locator('#search').fill('stop'); await page.waitForTimeout(230);
    ...
    await append(app, Array.from({ length: 80 }, (_, index) => frame({ index, session: `paged-session-${String(index).padStart(3, '0')}`, hook: index % 2 ? 'Stop' : 'PreToolUse' })));
    await page.locator('#session').focus();
    ...
    await expect(page.locator('#session option').filter({ hasText: 'paged-session-063' })).toHaveCount(1);
    await page.locator('#session').selectOption('@next');
    await expect(page.locator('#session option').filter({ hasText: 'paged-session-079' })).toHaveCount(1);
  ...
  } finally { await app.close(); await video.saveAs(info.outputPath('queries-walkthrough.webm')); }
});
...
test('recorded narrow failures: bounded long-ID choices, held filter offset, timeout, eviction and recovery', async ({}, info) => {
  ...
  try {
    await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].setContentSize(440, 820));
    const longEvents = Array.from({ length: 24 }, (_, index) => frame({ index,
      session: `long-session-${String(index).padStart(3, '0')}-${'x'.repeat(7000)}`, hook: 'Stop', message: 'narrow filter and recovery' }));
    await append(app, longEvents, 40);
    await page.locator('#session').focus();
    await expect(page.locator('#session option[value="@next"]')).toHaveCount(1);
    const choices = await page.locator('#session option').evaluateAll(nodes => nodes.filter(node => node.value && !node.value.startsWith('@')).map(node => JSON.parse(node.value)));
    expect(choices.length).toBeLessThan(24);
    expect(Buffer.byteLength(choices.join(''))).toBeLessThanOrEqual(128 * 1024);
    await page.locator('#session').selectOption('@next');
    await expect(page.locator('#session option').filter({ hasText: 'long-session-023-' })).toHaveCount(1);
    ...
    await expect(page.locator('#count')).toHaveAttribute('data-arrivals', '2');
    await expectSelected(page, held);
    expect(await page.locator('#entries').textContent()).toBe(rows);
    expect(await page.locator('#payload').evaluate(node => node.scrollTop)).toBe(offset);
    await capture(page, info, 'narrow-held-filter');
    await fault(app, { searchMs: 0 }); await page.locator('#search').fill('missing');
    await expect(page.locator('#notice')).toContainText('Search timed out');
    await capture(page, info, 'narrow-timeout');
    await fault(app, { searchMs: 250 });
    await page.locator('#notice').getByRole('button', { name: 'Reset filters' }).click();
    await expect(page.locator('#notice')).not.toContainText('Search timed out');
    await append(app, Array.from({ length: 150 }, () => source[4]), 75);
    await expect(page.locator('#notice')).toContainText('selected event was evicted');
    await capture(page, info, 'narrow-eviction');
    await fault(app, { disk: true }); await append(app, [source[4]], 80);
    await expect(page.locator('#notice')).toContainText('Storage pressure');
    ...
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  } finally { await app.close(); await video.saveAs(info.outputPath('narrow-walkthrough.webm')); }
});
...
test('a late navigation reply cannot display a target evicted after its database query', async ({}, info) => {
  ...
  try {
    ...
    const shown = await page.locator('.event').evaluateAll(nodes => nodes.map(node => Number(node.dataset.event)));
