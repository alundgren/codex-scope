...
const [hello, ...events] = fixtures.toString().trim().split('\n').map(JSON.parse);
const parse = messages => recording.parseRecording(Buffer.from(messages.map(message => JSON.stringify(message)).join('\n')));
const withRaw = (raw, sequence = 1) => ({ ...events[0], sequence, payload: raw, payload_bytes: Buffer.byteLength(raw) });
...
test('the independent adapter accepts the shared protocol fixture', async () => {
  const data = recording.parseRecording(await readFile(new URL('../../protocol/fixtures/stream.jsonl', import.meta.url)));
...
test('oversized, byte-mismatched and invalid payloads are dropped whole before retention', () => {
  const tooBig = { ...events[3], sequence: 1, payload: events[3].payload + ' ', payload_bytes: 61441 };
  const badBytes = { ...events[1], sequence: 2, payload_bytes: 1 };
  const malformed = withRaw('{"hook_event_name":', 3);
  const infinity = withRaw('{"hook_event_name":"SessionStart","session_id":"demo-a7","value":1e999}', 4);
  const mismatch = { ...events[0], sequence: 5, session_id: 'wrong' };
  const brokenUnicode = withRaw('{"hook_event_name":"SessionStart","session_id":"demo-a7","value":"\ud800"}', 6);
  const valid = { ...events[0], sequence: 7 };
  const data = parse([hello, tooBig, badBytes, malformed, infinity, mismatch, brokenUnicode, valid]);
  assert.equal(data.events.length, 1);
...
test('source, frame, event count and total payload bytes have independent limits', () => {
  assert.throws(() => recording.parseRecording(Buffer.alloc(recording.MAX_SOURCE_BYTES + 1)), /too large/);
  const source = Buffer.from(JSON.stringify(hello) + '\n' + 'x'.repeat(recording.MAX_FRAME_BYTES + 1) + '\n' + JSON.stringify(events[0]));
  assert.equal(recording.parseRecording(source).drops.oversized, 1);
  const count = parse([hello, ...Array.from({ length: 20 }, (_, index) => ({ ...events[0], sequence: index + 1 }))]);
  assert.equal(count.events.length, recording.MAX_EVENTS);
  assert.equal(count.drops.capacity, 4);
  const bytes = parse([hello, ...Array.from({ length: 6 }, (_, index) => ({ ...events[3], sequence: index + 1 }))]);
...
test('invalid headers, frame UTF-8 and request parameters cannot select payloads', () => {
  ...
  const data = recording.parseRecording(fixtures);
  for (const [id, rows] of [[0, 5], [1, 6], [1, 0], ['1', 5], [{}, 5], [1, NaN]]) {
    assert.throws(() => recording.inspect(data, id, rows));
  }
});
