...
import { fakeCollector, wait, until } from './fake-collector.mjs';
...
async function launch(info, server, token = 'synthetic-test-token', root) {
  ...
  const page = await app.firstWindow(); await page.waitForSelector('html[data-ready="true"]');
  return { app, page, root, video: page.video() };
}
async function capture(page, info, name) { await page.mouse.move(1, 1); await page.waitForTimeout(250); await page.screenshot({ path: info.outputPath(`${name}.png`) }); }
async function seed(server) { for (const event of source.slice(1)) { server.event(event); await wait(30); } }

test('recorded transport: connection, held reconnect, totals, local drops, Clear and hidden capture', async ({}, info) => {
  const server = await fakeCollector(), { app, page, video } = await launch(info, server);
  ...
  try {
    ...
    await seed(server); await expect(page.locator('#count')).toHaveText('5 retained');
    ...
    const offset = await page.locator('#payload').evaluate(node => node.scrollTop), rows = await page.locator('#entries').innerText();
    expect(offset).toBeGreaterThan(0);
    const initial = await app.evaluate(() => globalThis.scopeHistory.inspect(1, 4, 5));
    expect(initial.selected.text).toBe(source[4].payload); expect(initial.selected.bytes).toBe(61440);
    const oldConnection = (await status(app)).connectionId;
    server.state.status = 503; server.disconnect();
    await expect(page.locator('.connection')).toHaveText('Disconnected'); await expect(page.locator('#notice')).toContainText('Collector is busy');
    await capture(page, info, 'disconnected-held');
    expect(await page.locator('#entries').innerText()).toBe(rows); expect(await page.locator('#payload').evaluate(node => node.scrollTop)).toBe(offset);
    server.state.status = 200;
    await expect(page.locator('.connection')).toHaveText('Connected');
    expect((await status(app)).connectionId).not.toBe(oldConnection);
    server.event(source[1]); await expect(page.locator('#count')).toContainText('1 new');
    expect(await page.locator('#payload').getAttribute('data-event')).toBe('4'); expect(await page.locator('#payload').evaluate(node => node.scrollTop)).toBe(offset);
    await capture(page, info, 'reconnected-held');
    server.state.knownDrops.queue = 9; server.state.knownDrops.no_viewer = 12;
    await expect(page.locator('#notice')).toContainText('Collector lifetime drops: 12 no_viewer, 9 queue');
    await fault(app, { disk: true }); server.event(source[1]);
    ...
    await page.locator('#notice').focus(); await page.locator('#notice').press('End');
    await capture(page, info, 'local-drops-narrow');
    await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].setContentSize(1180, 760));
    await fault(app, { disk: false }); server.event(source[1]); await expect(page.locator('#notice')).not.toContainText('Storage pressure');
    server.state.knownDrops = { no_viewer: 0, invalid: 0, oversized: 0, rate: 0, queue: 0, disconnect: 0 };
    server.disconnect(); await expect.poll(async () => (await status(app)).transport?.collectorTotals?.queue).toBe(0);
    ...
    const connection = server.state.connectionId; server.event(source[2]);
    const pending = app.evaluate(() => globalThis.scopeHistory.inspect(1, 4, 5));
    await wait(100); await page.locator('#clear').click(); await page.locator('#clear').click();
    await expect(page.locator('#count')).toHaveText('0 retained'); await expect(page.locator('#json')).toBeEmpty();
    await expect.poll(async () => (await status(app)).generation).toBe(2);
    expect(await pending).toHaveProperty('stale', true);
    await until(() => server.state.connectionId !== connection); await wait(1300);
    expect((await status(app)).total).toBe(0); await capture(page, info, 'clear-reconnected');
    await fault(app, { transportDelay: 0, delay: 0 }); server.event(source[3]);
    await expect(page.locator('#count')).toHaveText('1 retained'); await expect(page.locator('#json')).toHaveText(source[3].payload);
    await capture(page, info, 'clear-recovered');
    await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].hide());
    for (let index = 0; index < 8; index++) { server.event(source[1]); await wait(350); }
    ...
    await page.locator('#copy').click(); expect(await app.evaluate(({ clipboard }) => clipboard.readText())).toBe(source[1].payload);
    await fault(app, { cleanup: true }); await page.locator('#clear').click(); await page.locator('#clear').click();
    ...
    await until(() => server.state.stream.destroyed); await capture(page, info, 'clear-cleanup-failed');
...
test('recorded authentication and second-viewer failures recover on configuration restart or free lease', async ({}, info) => {
  const server = await fakeCollector(); let run = await launch(info, server, 'wrong-synthetic-token');
  try {
    await expect(run.page.locator('#notice')).toContainText('Authentication failed'); await capture(run.page, info, 'authentication-failed');
    const requests = server.state.requestCount; await wait(1500); expect(server.state.requestCount).toBe(requests);
    await run.app.close(); await run.video.saveAs(info.outputPath('authentication-walkthrough.webm'));
    server.state.status = 409; run = await launch(info, server);
    await expect(run.page.locator('#notice')).toContainText('Another viewer is connected'); await capture(run.page, info, 'second-viewer');
    await run.app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].setContentSize(440, 820)); await capture(run.page, info, 'second-viewer-narrow');
    server.state.status = 200; await expect(run.page.locator('.connection')).toHaveText('Connected'); server.event(source[1]);
    await expect(run.page.locator('#count')).toHaveText('1 retained'); await capture(run.page, info, 'second-viewer-recovered');
    server.state.status = 401; server.disconnect();
    await expect(run.page.locator('#notice')).toContainText('Authentication failed');
    const beforeClear = server.state.requestCount;
    await run.page.locator('#clear').click(); await run.page.locator('#clear').click();
    await expect(run.page.locator('#count')).toHaveText('0 retained');
    await expect(run.page.locator('#notice')).toContainText('Authentication failed');
    await wait(1200); expect(server.state.requestCount).toBe(beforeClear);
    await capture(run.page, info, 'authentication-clear');
    await run.app.close(); await run.video.saveAs(info.outputPath('conflict-walkthrough.webm')); run = null;
...
test('recorded stalled storage releases lease and drops old transport work before recovery', async ({}, info) => {
  const server = await fakeCollector(), { app, page, video } = await launch(info, server);
  try {
    await expect(page.locator('.connection')).toHaveText('Connected'); await seed(server); await expect(page.locator('#count')).toHaveText('5 retained');
    await page.locator('button[data-event="4"]').click(); await page.locator('#scrollbar').press('PageDown');
    const offset = await page.locator('#payload').evaluate(node => node.scrollTop);
    await fault(app, { transportDelay: 6500 }); server.event(source[1]);
    await expect(page.locator('#notice')).toContainText('Intake stopped making progress'); await capture(page, info, 'stalled-storage');
    const requests = server.state.heartbeats; await wait(2100); expect(server.state.heartbeats).toBe(requests);
    await fault(app, { transportDelay: 0 }); await expect(page.locator('.connection')).toHaveText('Connected');
    server.event(source[1]); await expect(page.locator('#count')).toContainText('1 new'); expect((await status(app)).total).toBe(6);
