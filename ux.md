...
The overlay renders accepted content as plain text with bounded parsing. Original JSON and its copy action preserve the complete accepted hook text, including whitespace and unknown fields. Response and Input copy the displayed value. Do not format large payloads with indentation or inject HTML. The original payload remains available even if a response is missing. The payload scrolls with wheel, touch, keyboard, and the existing custom scrollbar. Keep its offset while capture continues. The scrollbar's thumb represents the visible fraction of content; track clicks, dragging, arrows, Page Up/Down and Home/End remain available. Hide it and remove it from keyboard navigation when content fits.
...
Use Codex Scope as the window title and Event journal as the journal title. Do not add UX subheadings. The table, summary rail, combined picker, and overlay follow the selected reference. Keep existing capture controls, Functions, and Clear. At narrow widths, put summaries beneath the table and keep the modal within the window. No permanent payload sidebar or timeline scrubber remains.
